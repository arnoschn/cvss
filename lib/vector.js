...
function normalize(vector) {
  ...
  for (key in vector) {
    ...
    value = vector[key];
    payload[key] =
      value in metrics.toScore[key] ? value : metrics.defaults[key];
  }
  return payload;
}
...
function validate(input) {
  ...
  if (type === "object" && input !== null) {
    ...
    err = new TypeError(
      "Vector must be an object or string but was a(n) " + type + " instead."
    );
  ...
  if (vector) {
    keys.base.every(function(key) {
      if (!vector[key]) {
        err = new Error(
          'Invalid CVSS3 "vector": missing required metric (' + key + ")"
        );
        return false;
      }
      return true;
