...
function getScore(input, opts) {
  ...
  var score = scores.getBase(
    validated.vector,
    isEnv ? { env: true } : undefined
  );

  if (
    !options.baseOnly &&
    validated.hasTemporal &&
    (options.env || options.temporal)
  ) {
...
function getBaseScore(input, options) {
  return exports.getScore(
    input,
    Object.assign({}, options, { baseOnly: true })
  );
}
...
function getTemporalScore(input, options) {
  return exports.getScore(
    input,
    Object.assign({}, options, { temporal: true })
  );
}
